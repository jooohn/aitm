...
- `domainResultToApiResult` in `src/backend/api/error-response.ts` bridges the two layers: it converts a `DomainResult<T, DomainError>` into the existing `ApiResult<T>` using `DomainError.statusCode`, so route handlers consume domain results with the same `.ok` / `.data` / `.response` pattern they already use.
- Migration is incremental. Non-migrated methods continue throwing; route handlers use try/catch for those and `domainResultToApiResult` for migrated ones. Both styles coexist.
...
- The bridge function keeps route handlers unchanged in structure — they still check `.ok` and return `.response` on failure.
