...
describe("registerRepository", () => {
  it("registers a valid git repo and returns the record", () => {
    ...
    expect(repo.name).toBe(repoPath.split("/").at(-1));
    expect(repo.main_branch).toBe("main");
    expect(repo.created_at).toMatch(/^\d{4}-\d{2}-\d{2}T/);
  });

  it("accepts explicit name and main_branch", () => {
    const repoPath = makeFakeGitRepo();
    const repo = registerRepository({ path: repoPath, name: "my-repo", main_branch: "master" });

    expect(repo.name).toBe("my-repo");
    expect(repo.main_branch).toBe("master");
  });
  ...
  it("rejects a path that is not a git repository", () => {
    const dir = makeTempDir();
    expect(() => registerRepository({ path: dir })).toThrow("not a git repository");
  ...
  it("rejects a duplicate path", () => {
    ...
    expect(() => registerRepository({ path: repoPath })).toThrow("already registered");
  });
});
...
describe("listRepositories", () => {
  ...
  it("returns all repos ordered by name ascending", () => {
    const a = makeFakeGitRepo();
    const b = makeFakeGitRepo();
    registerRepository({ path: a, name: "zebra" });
    registerRepository({ path: b, name: "alpha" });
